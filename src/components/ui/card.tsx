...
export function Card({ className, children, interactive = false, tone = 'default', ...props }: CardProps) {
  return (
    <section
      className={cn(
        'relative rounded-[1.125rem] border p-5 transition-[border-color,box-shadow,transform,background-color] duration-200 ease-out sm:p-6',
        tones[tone],
        interactive && 'cursor-pointer hover:-translate-y-0.5 hover:border-border-strong hover:shadow-card-hover',
        className,
      )}
      {...props}
    >
